...
class ReviewListViewTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Review.objects.create(
            title="Test Review 1", content="Test Content 1",
            user=test_user, approved=True)
        Review.objects.create(
            title="Test Review 2", content="Test Content 2",
            user=test_user2, approved=False)
    ...
    def test_review_list_view_with_login(self):
        """
        Test that authenticated users can see their own unapproved reviews in
        addition to the approved ones.
        """
